...
namespace Test1._1.Controllers
{
	public class ApplicantInfoController : Controller
	{
		...
		public IActionResult Index(string id)
		{
			...
			// جميع الاشتراكات المتاحة
			var subscriptions = _context.ApplicantSubscraptions.ToList();

			// الاشتراك النشط إن وجد
			var activeTransaction = _context.ApplicantTransactions
				.Include(t => t.ApplicantSubscraption)
				.FirstOrDefault(t => t.ApplicantId == id && t.IsPaid && t.IsActive);

			// إرسال البيانات إلى الـ View
			ViewBag.ApplicantSubscraptions = subscriptions;
			ViewBag.ActiveTransaction = activeTransaction;

			return View(applicant); // النموذج الرئيسي هو Applicant
		}
